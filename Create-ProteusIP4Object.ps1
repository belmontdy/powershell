...
.Description
Published 3/24/2020. Author David Belmont. Utilizing the Proteus BAM API this script can either create a single reservation or import a batch of reservations. Imports must be a csv format [mac,IPName,ObjectType] with no headers.

.Example
Single Import:

    Create-ProteusIP4Object -Subnet '172.100.10.0' -MacAddress '12:ab:12:ab:12:ab' -IP4Name 'TestCreate' -ObjectType MAKE_STATIC
...
Mass Import with Offset:
    ...
    Create-ProteusIP4Object -Subnet '172.100.10.0' -Offset '172.100.10.20' -CSVImport 'C:\temp\BAMImport.csv' -LogPath '\\someserver\share\BAMImport.log'
...
param(
    ...
    [string]$ProteusAPIURL = "http://[InsertYourServerNameHere]/Services/API?wsdl",
...
function Get-NextAvailableIP
{
    if($Offset -eq $null)
    {
        return $p1.getNextIP4Address("$($network.id)",$network.properties)
    }
    else
    {
